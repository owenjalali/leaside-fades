...
Decision:
Add `.github/workflows/send-reminders.yml` as a free scheduler path for production reminders while cron-job.org cannot be repaired without its API key. The workflow runs at low-congestion UTC minutes `13` and `43`, uses repository secret `LEASIDE_REMINDER_CRON_SECRET`, and fails on non-2xx responses. The HTTP reminder guard uses the durable `scheduler_job_runs` heartbeat for live cadence decisions, so delayed authorized scheduler runs execute when the last success is stale and skip cleanly when a recent success already satisfies the 30-minute cadence.
...
Decision:
`/admin/dashboard` revenue means tracked service-snapshot value for appointments that have happened: completed bookings plus confirmed bookings whose start time is current or past. Totals are summed from stored `booking_services.price_cents` snapshots on the appointment's `America/Toronto` local date. Week view uses seven daily buckets ending on the anchor date, Month view uses all days in the anchor month, Year view uses 12 monthly buckets, and All time spans the actor-scoped earliest/latest happened appointments with monthly buckets. Future confirmed, cancelled, and no-show bookings are excluded. When no anchor date is supplied for bounded periods, the server anchors the revenue card to the latest reportable historical appointment date for that actor scope. From-price services count at their stored snapshot totals with a dashboard caveat, and unpriced reportable appointments are counted separately. Completion remains a role-scoped status transition for current or past confirmed bookings and does not create payment records or lifecycle notifications.
...
### 2026-07-03 - Normalize Database Connection Strings To sslmode=verify-full

Decision:
Normalize the database connection string at client-creation time. `normalizeDatabaseUrl()` in `src/server/db/client.ts` rewrites `sslmode=require` to `sslmode=verify-full` on `DATABASE_URL` before the pool is created, and a `DATABASE_SSL_MODE` environment variable overrides the normalization when set.
...
### 2026-07-18 - Retry The Bounded Reminder Connect Once For Neon Cold Starts

Decision:
Give the reminder route's `database_connect` stage one bounded in-process retry. When the first `pool.connect()` attempt fails and at least 2.5 seconds of the 12-second initialization budget remain (a 500 ms delay plus a 2-second minimum attempt budget), the route closes the failed pool, creates a fresh one, waits 500 ms, and connects once more under the same initialization deadline. A second failure or an exhausted budget still returns the existing `503` with stage `database_connect`. Recommend `REMINDER_DB_CONNECT_TIMEOUT_MS=5000` in production and the environment examples so two connect attempts plus the retry delay (10.5 seconds) fit inside the initialization budget.

Reason:
Cold-start `503` responses persisted after the July 17 default connection-timeout bump because Vercel production sets `REMINDER_DB_CONNECT_TIMEOUT_MS` explicitly, and that environment override keeps the raised 8-second default from ever applying. A failed connect attempt still wakes the suspended Neon compute, so an immediate second attempt almost always succeeds — the GitHub canary proved this externally with `curl --retry 2`. Retrying inside the process absorbs the cold start without changing the external scheduler contract, the bounded budgets, or the overall HTTP deadline.
