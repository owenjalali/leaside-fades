...
Build a production-grade booking and scheduling platform for Leaside Fades with two locations, multiple barbers, shift-based availability, customer bookings, admin/barber management, and Brevo email plus optional Twilio SMS notifications.
...
- SMS: Twilio, independently pausable when funding is unavailable
...
- Business hours mark whether a location is open or closed for a local date. On open days, saved barber shifts and one-off add overrides define customer availability and may intentionally extend before posted opening or after posted close.
...
Responsible for:
- abstracting Twilio and Brevo providers
...
Phase 12/13 incident hardening adds `scheduler_job_runs` as a lightweight operational heartbeat for reminder jobs. Real reminder scheduler runs record success/failure, trigger, duration, result counts, and error details. Provider delivery failures and deadline-deferred work record a successful scheduler heartbeat with degraded result counts because the scheduler and database ran, while database/initialization/job infrastructure failures record a failed heartbeat. `/admin/dashboard` classifies the latest reminder heartbeat as healthy, degraded, stale, failing, or unknown and also exposes the configured Brevo/Twilio delivery state without secrets.

The production HTTP reminder endpoint is scheduler-agnostic: cron-job.org, GitHub Actions, or Vercel Cron may call the same secured route when configured with the current `CRON_SECRET`. It authenticates before importing database/job modules, then uses one PostgreSQL pool connection for cadence, reminder work, heartbeat, and retention. Connection, query, initialization, provider, and overall HTTP budgets are bounded; the default 24-second overall deadline stays below the host's 30-second limit. A PostgreSQL advisory lock prevents concurrent reminder runners. On quota-limited database plans, the route keeps a 30-minute database cadence by checking the durable reminder heartbeat before running. Delayed authorized scheduler calls still run when the last successful heartbeat is stale; duplicate calls skip when a recent success already satisfies the cadence.
...
Phase 12 originally wired Phase 5B password reset delivery to Resend. Production now uses the shared Brevo email provider. Non-production environments keep dev console logging. Production reset links require `APP_URL` and route to `/admin/reset-password`.

Phase 12 originally wired Phase 5C barber invite delivery to Resend. Production now uses the shared Brevo email provider. Non-production environments keep dev console logging. Production invite links require `APP_URL` and route to `/admin/accept-invite`.
...
5. Load location business hours for the local date and return no availability if the location is closed.
6. Load recurring shifts and one-off overrides for eligible barbers.
7. Use saved barber shift windows and one-off add overrides as the public bookable windows for open days.
...
Dashboard snapshot:
- `GET /api/admin/dashboard?period=week|month|year|all-time&anchorDate=YYYY-MM-DD` returns today's active appointments, upcoming active appointments, notification/reminder activity, tracked service-snapshot revenue, upcoming confirmed/cancelled appointment series, and notification health
...
- week revenue uses seven daily buckets ending on the anchor date, month revenue uses daily buckets for the anchor month, year revenue uses 12 monthly buckets for the anchor year, and all-time revenue spans the actor-scoped historical happened-appointment range with monthly buckets
- when `anchorDate` is omitted for bounded periods, the server uses the latest reportable historical revenue date for the actor scope instead of blindly anchoring to an empty current week; all-time ignores period navigation and uses the earliest/latest reportable happened appointments
...
Implemented:
- `/admin/dashboard` owner/staff landing surface with tracked service-snapshot revenue, Week/Month/Year/All time controls, upcoming confirmed/cancelled appointment trends, appointment activity, and compact notification health
...
- customer email through Brevo transactional email
- barber/staff email through Brevo for booking confirmations, cancellations, and reschedules
...
- `live`: Brevo email using `BREVO_API_KEY`, `EMAIL_FROM`, and optional `EMAIL_REPLY_TO`; Twilio SMS uses the stored credentials only when `SMS_DELIVERY_MODE=live`
- `SMS_DELIVERY_MODE=paused`: SMS attempts are recorded as `skipped` with `provider_paused`, no Twilio client is initialized, and later runs do not repeatedly fail the same reminder

Rules:
- booking mutations complete before notification dispatch starts
- Twilio/Brevo calls never run inside booking database transactions
...
- reminder jobs can run through `npm run notifications:send-reminders` or the authenticated HTTP cron endpoint; neither uses a long-lived Express timer
- production reminder cron should first pass `npm run notifications:check-live-config`
- reminder candidates are confirmed public/manual/walk-in bookings whose `start_time - 2 hours` falls within the configured due window
