...
- `/admin/dashboard` now presents a Fresha-inspired operating dashboard with completed appointment revenue from stored booking service price snapshots, Week/Month/Year period controls, upcoming confirmed/cancelled appointment trends, compact notification health, recent appointment activity, and 30-second polling that preserves the last good snapshot on refresh failure.
...
- Dashboard revenue means completed appointments only, summed from immutable `booking_services.price_cents` snapshots on the appointment's Toronto local date. It is not payment/POS revenue, does not create payment records, and excludes confirmed, cancelled, and no-show bookings.
...
- The premium dashboard redesign is the intended production dashboard surface. A read-only production dashboard snapshot on May 5, 2026 previously returned service-snapshot value data; the current dashboard now reports only completed service-snapshot revenue in the selected Week/Month/Year period.
...
June 9, 2026 dashboard revenue update: `/admin/dashboard` now accepts `period=week|month|year` and `anchorDate=YYYY-MM-DD`, and the React dashboard shows one adaptive completed-revenue card with Week/Month/Year controls plus previous/next period navigation. Revenue is completed appointments only, grouped by Toronto appointment date and summed from stored `booking_services.price_cents` snapshots; from-price services count at the stored snapshot total with a dashboard caveat, and unpriced completions are counted separately. `POST /api/admin/bookings/:bookingId/complete` lets owner/admin users complete scoped current/past confirmed bookings and barber users complete only their own; future, cancelled, completed, and no-show bookings reject completion. Verification passed: targeted admin/dashboard tests, full `npm run test`, and `npm run build`.
