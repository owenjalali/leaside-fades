...
The external reminder scheduler is not yet recovered. On May 20, 2026, `npm run qa:production-reminder-scheduler` found 11 recent cron hits to `/api/jobs/send-reminders`, all returning `401`, with the latest at `2026-05-20T17:34:56.786Z` on deployment `dpl_BULeBJBCg1K7VKQhXCYRuyjAVFZu`. That means cron-job.org is reaching production but its Authorization header is missing, stale, or not matching the current Vercel Production `CRON_SECRET`.
...
Vercel production still shows encrypted `CRON_SECRET` metadata, but `vercel env pull --environment=production` currently returns it locally as an empty quoted value, and the ignored local `.env.production.local` value is not accepted by production dry-run. A real current production cron secret must be retrieved or rotated before cron-job.org can be repaired.
