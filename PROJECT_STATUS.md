...
July 16 follow-up verification closed the two remaining QA gaps. The authenticated cron-job.org history shows the reminder job enabled with five consecutive `200 OK` executions through the 12:00 PM run; that latest run completed in 882 ms, the 11:30 AM run completed in 1.19 seconds, and the next run was scheduled for 12:30 PM. The older 30-second timeout rows remain only as historical evidence from before the hardening. Local Docker Desktop and the preserved `leaside-fades-postgres` container were restored, the container restart policy was changed to `unless-stopped`, the current 30-day local sample-shift window was reseeded, and `npm run qa:phase9-notifications` passed with full cleanup. Its lifecycle expectations now match the approved cost model: booking confirmation, cancellation, reschedule, and walk-in lifecycle events are email-only, while SMS remains reserved for reminder events and is currently paused.

July 17 mailbox, cron-job.org, Vercel, durable-heartbeat, and Neon inspection found a second scheduler issue that the initial green window did not expose. Fresh cron-job.org alerts were genuine intermittent `503 Service Unavailable` responses, alternating with successful runs. The saved 9:00 AM execution waited 6.52 seconds and received the application's generic `Reminder service initialization timed out` response; no failed scheduler row was recorded because the request stopped before job execution. Neon showed the Free-plan compute had suspended between runs and restarted later, while the route still allowed only 4 seconds for a cold database connection. The correction raises only the database-connect default to 8 seconds while preserving the 12-second initialization cap and the 24-second overall HTTP deadline. Initialization failures now expose a fixed safe stage (`client_creation`, `database_connect`, `advisory_lock`, or `scheduler_summary`) in the authenticated response and Vercel log without exposing credentials, URLs, SQL, or provider bodies.

The correction is live from production deployment `dpl_9PTYz7uJwERrEpbNfdNtyHswGgdE`. After leaving Neon idle beyond its suspend window, cron-job.org's first post-deploy external execution ran at 9:30:19 AM and returned `200 OK` in 5.10 seconds. Its saved response was healthy (`degraded=false`, `failed=0`, `deferred=0`); the due Brevo email sent and Twilio attempts were safely recorded as paused skips. The non-mutating production smoke also passed on the same deployment.
