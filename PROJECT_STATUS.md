...
- A secured `GET /api/jobs/send-reminders` endpoint exists for reminder schedulers and requires `CRON_SECRET` before it will run. Production returns `401` without the secret, confirming `CRON_SECRET` is present at runtime. Vercel Hobby blocked the desired five-minute cron registration, so production scheduling requires Vercel Pro or an external scheduler.
- Vercel production contains encrypted Twilio/Resend notification environment variables, but the local Vercel CLI cannot inspect sensitive values for `npm run notifications:check-live-config`. A controlled live SMS/email smoke test has not been sent yet.
...
Continue Phase 12/13 by choosing Vercel Pro or an external five-minute reminder scheduler, running controlled production booking/notification smoke tests with owner-approved test contacts, verifying Google/social production links, and obtaining final owner signoff.
...
- Production Twilio and Resend credentials before live notification delivery.
- A five-minute reminder scheduler. Vercel Hobby blocks the desired Vercel Cron frequency, so launch needs Vercel Pro or an external scheduler.
- Live production checks for Google Places, social links, Resend, Twilio, controlled notifications, and reminder scheduler enablement are still Phase 12 launch-prep work.
