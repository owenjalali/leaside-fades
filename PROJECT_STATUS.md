...
The external reminder scheduler is not yet recovered. On May 20, 2026, `npm run qa:production-reminder-scheduler` found 16 recent cron hits to `/api/jobs/send-reminders`, all returning `401`, with the latest at `2026-05-20T17:05:31.196Z` on deployment `dpl_4RFM9swArTaKMmRL2nceXrCmgMcr`. That means cron-job.org is reaching production but its Authorization header is missing, stale, or not matching the current Vercel Production `CRON_SECRET`.
