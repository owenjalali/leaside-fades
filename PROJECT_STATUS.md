...
- `/admin/shifts` now uses a staff-first weekly schedule builder inspired by the approved Prototype A concept: pick one staff member, edit working days/time windows/location inline, review weekly hours/effective dates, save changes explicitly, and keep one-off overrides plus team overview in secondary tabs. The builder displays the latest dated recurring pattern when multiple active effective ranges exist for the same staff member, avoiding duplicated day windows in the editor.
